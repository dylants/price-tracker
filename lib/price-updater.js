...
function PriceUpdater() {
    ...
    this.gatherItemDetails = function(uri, callback) {
        // certain sites require special actions to get the item
        // details, instead of just page scraping.
        if (uri.indexOf("store.sonyentertainmentnetwork.com") > -1) {
            findItemDetailsForSonyEntertainment(uri, callback);
            return;
        }

        // page scrape all the remaining sites
    ...
    function findItemDetailsForSonyEntertainment(uri, callback) {
        var apiURI, itemData;

        apiURI = "https://store.sonyentertainmentnetwork.com/store/api/chihiro/00_09_000/container/US/en/999/";
        // add the CID of the URI to the API URI
        apiURI = apiURI + uri.slice(uri.indexOf("cid=") + "cid=".length);

        async.whilst(
            function() {
                // run this until we get the item data
                return !itemData;
            },
            function(whilstCallback) {
                // scrape the site to find the item details
                request({
                    ...
                    if (response) {
                        if (response.statusCode === 200) {
                            // good response, grab the item data
            ...
            function(err) {
                var itemDetails, price, category, subcategory, name;
                ...
                if (itemData.default_sku.rewards && itemData.default_sku.rewards.length > 0) {
                    price = itemData.default_sku.rewards[0].display_price;
                    ...
                    price = itemData.default_sku.display_price;
                }

                if (!price) {
                    console.error("price was not found on sony store page, uri: " + uri);
                    callback("unable to find price for uri: " + uri);
                    return;
                }
                ...
                category = itemData.bucket;
                if (!category || category.length < 1) {
                    // bail, we don't have support for finding it anywhere else
                    console.error("category not found on sony store page, uri: " + uri);
                } else {
                    if (category === "games") {
                        category = CATEGORY_VIDEO_GAMES;
                    } else {
                ...
                // ******* SUBCATEGORY ******* //
                // *************************** //
                if (itemData.playable_platform && itemData.playable_platform.length > 0) {
                    subcategory = itemData.playable_platform[0];
                ...
                if (!name || name.length < 1) {
                    // bail, we don't have support for finding it anywhere else
                    console.error("name not found on sony store page, uri: " + uri);
                } else {
                    console.log("name: " + name);

...
var MIN_PERCENTAGE_CHANGE_NEEDED_TO_UPDATE = 0.05;
...
function PriceUpdater() {
        ...
        TrackedItem.find().populate("prices").exec(function(err, trackedItems) {
            ...
            async.whilst(
                ...
                function(whilstCallback) {
                    ...
                    console.log(" -- processing tracked item " + (count + 1) +
                    ...
                    async.waterfall([
                        ...
                        function(itemDetails, waterfallCallback) {
                            ...
                            // check to see if we received any item details
                            if (!itemDetails) {
                                // if we haven't any item details, nothing
                                // to do here, move on
                                console.log("no itemDetails found, moving on to next item");
                                waterfallCallback();
                                return;
                            }
                        ...
                        function(waterfallCallback) {
                            // if we're done, don't sleep
                            if (count === trackedItems.length) {
                                waterfallCallback();
                            } else {
                                // before finding details on the next tracked item,
                                // sleep anywhere between 1 second and 30 seconds in
                                // an attempt (which maybe futile) to make ourselves
                                // look less robotic to websites
                                var sleepTime = utils.generateNum(1000, 30000);
    ...
    function determineBestItemDetails(trackedItem, callback) {
         ...
         */
        async.eachSeries(trackedItem.uris, function(uri, eachCallback) {
            priceUpdater.gatherItemDetails(uri, function(err, currentItemDetails) {
                if (err) {
                    // if we've got an error getting the item details,
                    // log this error, but continue on -- don't let this
                    // halt all execution
                    console.error("*********");
                    console.error("error gathering item details for tracked item: " +
                        trackedItem.name);
                    console.error("uri: " + uri);
                    console.error(err);
                    console.error("*********");

                    console.log("...continuing to next uri...");
